...
| `--kv-cache-dtype turboquant_k8v4`                             | TurboQuant FP8-keys + 4-bit-values, 2.6x KV vs FP16, near-lossless (NIAH 100%, GSM8K 0.860). Set `KV_CACHE_DTYPE=fp8` to opt out. Requires `vllm>=0.20`. |
...
| `MAX_MODEL_LEN`        | `65536` (4090, auto) / `131072` (A6000 48G) | `docker-entrypoint.sh` auto-clamps to `65536` on single GPU when the upstream image default (`200000`) is detected. Doubling vs the previous `32768` is unlocked by `turboquant_k8v4` KV. Set explicitly to override (e.g. `131072` on 48 GB).                                                                                                                                                                                  |
...
| `KV_CACHE_DTYPE`       | `turboquant_k8v4`                           | KV cache quantization. The image bakes `turboquant_k8v4` (FP8 keys + 4-bit values, 2.6x compression vs FP16, near-lossless: NIAH 100%, GSM8K 0.860 — vLLM PR #38479, requires `vllm>=0.20`). More aggressive presets if you need >64K on a single 24 GB card: `turboquant_4bit_nc` (~3.8x), `turboquant_k3v4_nc` (~4.3x), `turboquant_3bit_nc` (~4.9x, GSM8K drops to 0.785). Set `fp8` to opt out (~2x, the previous default). |
...
- **24 GB 4090 is tight** with MTP spec-decode + 64K context. If you OOM at boot, switch to `KV_CACHE_DTYPE=turboquant_4bit_nc` (more headroom at small quality cost), drop `num_speculative_tokens` to `2` (edit `docker-entrypoint.sh`), or lower `GPU_MEMORY_UTIL` to `0.93`.
...
- **TurboQuant KV cache** — the image bakes `--kv-cache-dtype turboquant_k8v4` (FP8 keys + 4-bit values, vLLM PR #38479) which compresses KV pages to 196 B vs FP16's 512 B (2.6x). TurboQuant compresses only full-attention layers; Qwen3.6's GDN/Mamba state is unaffected. First-token latency is unchanged; per-token decode adds ~2–5% overhead in realistic configs (0xSero RTX 5090 27B-AWQ benchmark). Quality is statistically equivalent to FP16 baseline at `k8v4` (NIAH 100% match, GSM8K within noise). Set `KV_CACHE_DTYPE=fp8` to opt out.
