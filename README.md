...
| `MAX_NUM_BATCHED_TOKENS`              | `4128`                              | Prefill chunk size; auto-scaled by GPU count (TP=2 → 8192, TP≥4 → 16384) |
...
| `--max-num-batched-tokens 4128`                                | Prefill chunk size with `--enable-chunked-prefill`; matches vLLM's CUDA-graph capture endpoint (`4096 + 32`). `docker-entrypoint.sh` auto-scales by detected GPU count when the baked sentinel `4128` is still in effect: TP=1 stays at `4128`, TP=2 → `8192`, TP≥4 → `16384`. Override with `MAX_NUM_BATCHED_TOKENS` (e.g. `8192` on 1×A6000 48 GB for ~halved TTFT at ~600 MB extra activation). Raising past `4128` on a single 24 GB card tends to OOM at cold start. |
...
| `MAX_NUM_BATCHED_TOKENS` | `4128` (1×GPU, auto) / `8192` (TP=2) / `16384` (TP≥4) | Prefill chunk size with `--enable-chunked-prefill`. `4128` (= `4096 + 32`, vLLM's CUDA-graph capture endpoint) is what fits on 1×4090 24 GB alongside the fp8 KV pool at MML 32768. `docker-entrypoint.sh` auto-scales by detected GPU count when the baked sentinel `4128` is still in effect — sharded weights at TP≥2 free up per-GPU memory for larger activation buffers. On 1×A6000 48 GB you can override to `8192` for ~halved TTFT on long prompts (~600 MB more activation). Raising past `4128` on 24 GB tends to OOM at cold start (vLLM reserves activation buffers sized to this value during `profile_run`). |
